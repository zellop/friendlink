...
<head>
	...
	<body>
		<center>
			...
			<!-----搜索表单--------->
				<form action="newadminindex.php" method="get">
					...
					<input type="button" value="全部信息" onclick="window.location='newadminindex.php'"/>
				</form>
			<!-------------->

		<table style="width:100%;" border="1">
				<tr>
					<th>编号id</th><th>增加时间</th><th>关键词</th><th>链接</th><th>到期时间</th><th>商户</th>
					<th>对话</th><th>客服</th><th>增加链接</th><th>交易状态</th><th>查看</th><th>位置</th>
					<th>操作</th>
                   <!-- <th>排序</th>-->
				</tr>
				<?php
					...
					//3. 执行查询，并返回结果集
						...
						@$sql = "select * from linkdb {$where} order by duetime2 asc {$limit}";
					...
					while($row = mysql_fetch_assoc($result)){  
							...
							echo "<td style='word-break:break-all' width='6%'>".date("Y-m-d",$row['uptime2'])."</td>";

							echo "<td style='word-break:break-all' width='10%'>{$row['keyword']}</td>";
							echo "<td style='word-break:break-all' width='20%'>{$row['link']}</td>";
                            echo "<td style='word-break:break-all' width='10%'>到期时间 :  ".date("Y-m-d",$row['duetime2'])."</td>";
							...
							echo "<td style='word-break:break-all' width='5%'>{$row['status']}</td>";
							echo "<td style='word-break:break-all' width='3%'><a href='http://web.jmmama.com.cn/jmwebpage/link/out/answerpage6.php?swid=".$row['id']." ' target='_blank'>查看</a></td>";
							echo "<td style='word-break:break-all' width='3%'>{$row['linkout']}</td>";
							echo "<td style='word-break:break-all' width='5%'>
								<a href='javascript:dodel({$row['id']})'>删除</a>
								<a href='edit2.php?id={$row['id']}'>修改</a></td>";
                        /*echo "<td style='word-break:break-all' width='3%'>{$row['uptime']}</td>";*/
							echo "</tr>";
						}
					//5. 释放结果集
			 ...
			 <?php
                ...
                if($page == '1'){
                    ...
                    echo " <a href='newadminindex.php?page=1{$url}'>首页</a> ";
					echo " <a href='newadminindex.php?page=".($page-1)."{$url}'>上一页</a> ";
                }

                echo "<span class='navinfo'>当前{$page}/{$maxPages}页</span>";
                if($page == $maxPages){
                    
                }else{
                    echo " <a href='newadminindex.php?page=".($page+1)."{$url}'>下一页</a> ";
					echo " <a href='newadminindex.php?page={$maxPages}{$url}'>末页</a> ";
                }

                echo "<span class='navinfo'>共计{$maxRows}条</span>";
